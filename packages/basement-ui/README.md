...
| `toast.js` | `BasementToast` | Transient status at the top of the page, or the top of `.overlay-root`; `data-toast-duration` in ms, `0` stays until closed |
...
**Sheet** — `.sheet-host` + `.sheet`, anchored to the bottom. Content height hugs the content up to a cap that leaves a strip of the surface behind visible. `.sheet--half` is half the height of the page or overlay root. `.sheet-body` scrolls; `.sheet-heading` and `.sheet-actions` stay put. Closed, it takes no flow space. Same `data-overlay-dismiss` values as Dialog. `data-sheet-static` keeps a sample from closing.

**Toast** — `.toast-host` + `.toast` (`role="status"`). A translucent Gray chip with white text, no border, and no shadow, in both themes. Pinned to the top center of the page, or of an overlay root. The host does not block pointer events around the chip. `data-toast-duration` is milliseconds; `0` or omitting it leaves the toast up until the app closes it. `data-toast-close` may name a host id.

**Mockup** — `.mockup--phone` (23.4375rem × 50.75rem), `.mockup--tablet` (48rem × 64rem), `.mockup--desktop` (80rem × 45rem). `.mockup--landscape` swaps phone and tablet; desktop stays a landscape window. Phone and tablet are a hairline frame with Radius L and a narrow bezel. Desktop uses Radius M and a 2rem title bar (`.mockup-bar` / `.mockup-title`). `.mockup-screen` is the slot; add `.overlay-root` so sheets, dialogs, and toasts stay on the screen. Content lays out at the logical size. `.mockup-stage` scales the frame down with `zoom` to fit a sized parent and never past 1:1. Grayscale and the sans base face; no device hardware.
