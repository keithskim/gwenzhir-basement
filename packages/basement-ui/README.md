...
| `composer.js` | `BasementComposer` | Grows the chat composer field with its text where `field-sizing` is unsupported |
...
**Tag** — Outline, filled, split, and compact labels. `.tag--counter` is a compact Gray XD count. One digit is square (Radius S) at every size; extra digits grow wider. Uses lining proportional digits. Defaults to 2XS; pair with a size class to scale. `.tag-button` is a `<button>` that also has `.tag` classes. It keeps the tag’s type, uses the pointer, dims when disabled, and takes the same focus ring as Button. Static tags stay non-interactive.

**Image** — `img.img-placeholder`, or `.img-placeholder` around an image, turns a picture into a faint high-contrast grayscale on Gray XL. For stand-in photos in prototypes. With no picture, the frame is that pale gray. The reference shows the treatment on photographs under Graphics. Add `.img-placeholder--dither` to the wrapper for a two-tone 4×4 ordered dither in pure CSS: Gray dots on the surface color. `--img-dither-level` sets the dot gray (0.54), `--img-dither-cell` the cell size (1px), and `--img-dither-tone` the photo contrast before dithering (1.4).
...
**Button** — `.btn` with size (`--3xs` / `--2xs` / `--xs` / `--s`) and type (`--default` / `--subtle` / `--accent` / `--ghost`). `.btn--icon` is square; `.btn--round` is circular. `.btn--float` pins a filled control above everything else (`position: fixed`, end-bottom, `z-index` above Dialog). Add a text label beside the icon; omit `.btn--accent` for a filled Default surface. Hover stays opaque (Gray Extra Light / Extra Dark), never alpha. `data-presentation-state="hover"` or `"active"` paints that control’s hover or active look for a reference, walkthrough, test, or recording. It does not press, toggle, or move focus.
...
**Mockup** — `.mockup--phone` (21rem × 45.5rem), `.mockup--tablet` (48rem × 64rem), `.mockup--desktop` (80rem × 45rem). `.mockup--landscape` swaps phone and tablet and insets the screen from the side borders; desktop stays a landscape window. Phone and tablet are a hairline frame with Radius L and no inner padding. In portrait the screen is flush with the frame; a full-width bar such as a chat header or `.mockup-keyboard` pads the edge it meets. Sideways, the screen stays inset from the side borders. Desktop uses Radius M and a 2rem title bar (`.mockup-bar` / `.mockup-title`). `.mockup-screen` is the slot; add `.overlay-root` so sheets, dialogs, and toasts stay on the screen. Content lays out at the logical size. `.mockup-stage` scales the frame down with `zoom` to fit a sized parent and never past 1:1. `--mockup-zoom` is that scale. `--hairline` is `max(1px, 1px / var(--mockup-zoom))` inside the frame, so a one-pixel line does not paint thinner than that. Grayscale and the sans base face; no device hardware. Control markers sit on the screen. `.control-marker--tap` is a translucent gray circle with no border that pulses in place. `.control-marker--drag` is the same circle: it fades in like the tap, then travels from the start to the end. Set `--x` and `--y` from 0 to 100 across and down the screen; a drag also sets `--x2` and `--y2`. `.mockup-status` is the phone status row: a time, then Phosphor signal and battery icons. `.mockup-keyboard` sits along the bottom of `.mockup-screen`: rows of unlabeled white keys with a Light Gray border and a wider bar, with no letters or icons. `.mockup-keyboard-body` clips the motion and `.mockup-keyboard-pad` is the white deck. The keyboard is open by default. Add `.is-closed` to slide it out; remove the class to slide it back in.
...
**Chat rooms** — `.chat-rooms` is the conversation list (portrait, name, preview, time, optional unread Tag). It hugs its columns (XS inline padding, `max-content` width). Optional `.chat-room-group` / `.chat-room-label` cluster rooms; a leading `.checkbox` then `.rating-star` select and star. A checked room uses the same highlight as `.is-active`. When a room has those controls, the row is a `div`, `.chat-room-main` opens the thread, and the line is name, optional label, time, then optional unread counter. Times are Gray, lighter than the preview.

**Messages** — `.chat` is a thread: header and a scrollable log. Messages use Avatar, name, time, and body; `.chat-message--follow` hides the portrait and name for a run from the same person; `.chat-message--self` places the portrait, name, and time on the trailing edge. In Bubble, every note uses the same white surface and border, including outgoing notes. `.chat--bubble` wraps every note in a surface only as wide as the note, including when the note wraps. Message text uses compact leading (size + 0.25rem). Times are Gray, as in Chat rooms. `.chat-message--meta-inline` places `.chat-message-time` in its own column beside `.chat-message-text`, inside `.chat-message-row`: after the note for others, before it for yours. Your own beside notes skip the portrait and name; add `.chat-message--identified` to show them. `--chat-bubble-max` caps the note (16rem). `--chat-meta-gap` is the space between the surface and the time. The time column is the time’s own width, so a short wrapped note does not stretch to the log. `.chat-typing` is a log row that holds a Dots spinner (`aria-label` on the row names who is typing). An empty log uses `.chat-empty`. A rich note is a `.chat-card` (optional `.chat-card-media` or centered `.chat-card-mark`, title, copy, optional Pairs, `.chat-card-actions` stacked or `.chat-card-actions--row`). `--chat-card-max` caps it at 16rem, and the body inset is XS. A `.btn--default` in the log is Subtle; Accent and Ghost still opt in. `.chat-choice` uses that same Subtle surface. `.chat-card--list` is a header, rows of `.chat-card-thumb`, `.chat-card-label`, and `.chat-card-value`, then a footer total. Padding matches a bubble. `--chat-card-thumb` sets the thumbnail (1.75rem). `.chat-card--compact` hugs the content; without it the card keeps the usual width. `.chat-attachment-rail` is a horizontal row of cards in the message, starting after the avatar. `.chat-attachment-rail--bleed` extends the scrollport through the log’s trailing padding to the room edge. Cards snap; `--chat-card-basis` sets each card’s width (7rem). Titles on the rail are XS. Load `edge-fade.js` so the rail fades only while it overflows. `.chat-choices` is a stack of `.chat-choice` buttons in the thread: a label, an optional `.chat-choice-value`, and `.chat-choice-index` in a fixed column when there are two or more. A single choice omits the number. Nothing is selected by default. Disabled choices dim. Focus matches Button. `.chat-menu` is a panel pinned to the bottom of `.chat`, not a message. `.chat-menu-list` is full-width text rows with no icons. `.chat-menu-grid` is a rich menu (icon over label): three columns by default, `.chat-menu-grid--1` for one column, one or two rows; items past that are not shown. `.chat-menu` has a full border and top-corner radius; the chat clips the side and bottom borders so the toggle shows a rounded top. `.chat-menu-toggle` is a short full-width bar; it reads Menu unless the app sets another label. `.is-collapsed` hides the body and leaves that toggle. A `.toast-host` in the room sits at the bottom center, above the composer or the menu. A rich note stays within `--chat-card-max`.

**Composition** — `.chat-composer` is the field under a thread (a one-row `textarea.input` + Button). The field grows a line at a time up to `--chat-composer-lines` (5), then scrolls; `composer.js` does the sizing where `field-sizing` is unsupported. `.chat-suggestions` is a wrapping row of short reply chips above that field; apps choose the words from context. `.chat-frame` places Chat rooms beside a thread. Chrome only — apps own sending and delivery.
