...
- **Components** — Button (Default, Subtle, Accent, Ghost, Icon + disabled state; 2XS / XS / S), Tabs (subtle grouped controls, minimal underline), Toggle (horizontal toggle, checkbox, radio button, segmented select), Text Input (default, invisible inline editing, search, attached actions, textarea), Form (Input, Label, Textarea, Checkbox, Radio, Switch), Tag (default, filled; 2XS / XS / S / M), Avatar, Dropdown, Table (header, sortable columns, plain rows by default, optional striped rows), Sidebar (default, compact; menu items with count or icon and label), Alert (Neutral, Affirm, Warn, Destructive)
