...
- **Typography** — Base Text: Pretendard 600. Six sizes XS–2XL (0.75–4rem). Line height = font-size + 0.5rem by default; Compact (`type-compact`) uses font-size + 0.25rem for dense UI (tables, control labels).
...
- **Base** — Button (Default, Subtle, Accent, Ghost, Icon only, With icon; 2XS / XS / S; optional trailing compact tag for shortcuts), Tag (default, filled, split, compact; 2XS / XS / S / M), Status Chip, Mention, Avatar, Progress (optional value; fill amount via custom property), Spinner (Gray L ring with open gap; S / M / L; continuous spin)
- **Interactivity** — Tabs (subtle grouped controls, minimal underline, optional equal-width fill; collapses into a vertical list when labels exceed the container), Toggle (horizontal toggle, checkbox, radio button, segmented select), Text Input (default, invisible inline editing, search, attached actions, textarea), Form (Input, Label, Textarea, Checkbox, Radio, Switch), Menu, Dropdown, Sidebar (default, compact; menu items with count or icon and label), Prompt
- **Visualization** — Table (header, sortable columns, plain rows by default, optional striped rows, optional selectable rows with hover; compact tags in cells), Timeline, Graph (Bar: vertical and horizontal; muted and accent fills; bar label and value outside by default, each optionally inside; minimalist grid), Player (Default audio: play control, scrubber, elapsed and duration), Alert (Neutral, Affirm, Warn, Destructive)
...
- **Typography** — Typeface, Sizes, Headings (H1–H4), Body (paragraph S / M, quote S / M), Semantic text (Affirm, Warn, Destructive), Caption, Compact (tighter leading for dense UI), Links
